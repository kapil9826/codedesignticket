...
const TicketDetail: React.FC<TicketDetailProps> = ({ ticketId, onClose, onTicketChange }) => {
  ...
  // Dynamic ticket data based on ticketId (currently unused - keeping for future use)
  // const getTicketData = (id: string) => {
    ...
    return ticketData[ticketId] || ticketData['TC-0004'];
  };
  ...
  // const customer = getCustomerData(ticketId); // Currently unused
  ...
  // const attachments = getAttachmentsData(ticketId); // Currently unused
  ...
  const fetchStatuses = async () => {
    try {
      ...
      if (result.success && result.data && result.data.status === '1') {
        ...
        console.log('🎨 TicketDetail status colors:', result.data.data?.map((s: any) => ({ name: s.name, bg_color: s.bg_color, text_color: s.text_color })));
            ...
            <div className="attachments-list">
              {ticket.documents && ticket.documents.length > 0 ? (
                ...
                <div className="no-attachments">
                  <span style={{fontSize:"0.5rem"}}>No attachments available</span>
                </div>
              )}
            </div>
          )}
